...
#define EDGE_MAX 600001 // 양방향 간선이므로 30만개
...
Edge* pop(priorityQueue* pq)
{
	...
	while (leftChild < pq->count)
	{
		if (pq->heap[target]->cost > pq->heap[leftChild]->cost)
			target = leftChild;
		if (pq->heap[target]->cost > pq->heap[rightChild] && rightChild < pq->count)
			target = rightChild;
		if (target == now) break;
		else
		{
...
int main()
{
	...
	while (1)
	{
		...
		if (ans[curNode] < curCost) continue; // 의미가 없으므로
